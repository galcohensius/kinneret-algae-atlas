...
This document is for people working on the code. Updating the published atlas from Word files is in [MAINTENANCE.md](MAINTENANCE.md). Draft plans and review specs: [ideas/](../ideas/).
...
Word files are the source of truth; every JSON, image and discovery file downstream is regenerated, committed, and checked for drift in CI (Continuous Integration). The detailed pipeline diagram and Word-update checklist live in [MAINTENANCE.md](MAINTENANCE.md).
