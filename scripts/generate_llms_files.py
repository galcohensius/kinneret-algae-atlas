...
def _build_atlas_api(study_area: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "study_area": {
            ...
            "map_image": study_area.get("map_image"),
            "map_attribution": study_area.get("map_attribution"),
            "map_source_url": study_area.get("map_source_url"),
        },
        "citation": {
...
def _build_llms_txt(study_area: dict[str, Any]) -> str:
    lines = [
        ...
        "",
        f"Canonical URL: {ATLAS_CITE_URL}",
        f"Atlas attribution: {_atlas_attribution()}",
        "",
        *_study_area_block(study_area),
        "Primary resources:",
        f"- Species index (default: by phylum): {ATLAS_URL}/#algae-index",
        f"- Species index, morphotype view (by cell/colony shape): {ATLAS_URL}/#visual-index",
        f"- About: {ATLAS_URL}/about/",
        f"- Species pages: {ATLAS_URL}/algae/{{slug}}/",
        f"- Glossary (terms plus the Cox (1996) plates): {ATLAS_URL}/glossary/",
        f"- Supplementary material: {ATLAS_URL}/supplements/",
        f"- Sitemap: {ATLAS_URL}/sitemap.xml",
        "",
        "Machine-readable endpoints:",
        f"- {ATLAS_URL}/api/species.json",
        f"- {ATLAS_URL}/api/species/{{slug}}.json",
        f"- {ATLAS_URL}/api/glossary.json",
        f"- {ATLAS_URL}/api/atlas.json",
        f"- {ATLAS_URL}/api/search-index.json (internal: powers the site search box; "
        "schema is not stable, prefer species.json)",
        "",
        "Compact corpus:",
